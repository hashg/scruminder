...
Ember.SimpleAuth.setup = function(app, options) {
  ...
  Ember.$.ajaxPrefilter(function(options, originalOptions, jqXHR) {
    if (!jqXHR.crossDomain && !Ember.isEmpty(session.get('authToken'))) {
      jqXHR.setRequestHeader('Authorization', 'Basic ' + btoa(session.get('authToken') + ":") );
    }
  });
};
      ...
      if (!Ember.isEmpty(data.identification) && !Ember.isEmpty(data.password)) {
        ...
        Ember.$.ajax(Ember.SimpleAuth.serverSessionRoute, {
          ...
          if (data) {
            if (data.status === 'ERR')
            {
              Ember.tryInvoke(self, 'loginFailed', data.issues);
            }
            else
            {
          ...
          Ember.tryInvoke(self, 'systemFailed', arguments);
        });
      }
    }
  }
